...
using StorageMaister.Entities.Factories;
using StorageMaister.Entities.Products;
using StorageMaister.Entities.Storage;
using StorageMaister.Entities.Vehicles;
using System;
using System.Reflection;

namespace StorageMaister.Tests.Structure
{
    ...
    public class TestStructure
    {
        ...
        public void InvalidProductFactory()
        {
            ...
            Assert.Throws<InvalidOperationException>(() => productFactory.CreateProduct("gagaguga", 2.50));
        }
        ...
        public void InvalidVehicleFactory()
        {
            ...
            Assert.Throws<InvalidOperationException>(() => vehicleFactory.CreateVehicle("Gagaguga"));
        }
        ...
        {
            Type productExists = typeof(Product);
            PropertyInfo propertyInfo1 = productExists.GetProperty("Price");
            PropertyInfo propertyInfo2 = productExists.GetProperty("Weight");

            Assert.AreEqual(true, propertyInfo1.SetMethod.IsPrivate);
        }

        [Test]
        public void ProductPriceProperty()
        {
            Product Gpu = new Gpu(1.7);

            Assert.AreEqual(1.7, Gpu.Price);
        }

        [Test]
        public void ProductPricePropertyShouldThrow()
        {
            Assert.Throws<InvalidOperationException>(() => { Product Gpu = new Gpu(-3.2); });
        }
        ...
        {
            Type productExists = typeof(Storage);
            FieldInfo fieldInfo1 = productExists.GetField("garage", BindingFlags.Instance | BindingFlags.NonPublic);
            FieldInfo fieldInfo2 = productExists.GetField("products", BindingFlags.Instance | BindingFlags.NonPublic);

            Assert.AreEqual("garage", fieldInfo1.Name);
            Assert.AreEqual("products", fieldInfo2.Name);
            Assert.AreEqual(true, fieldInfo1.IsInitOnly);
            Assert.AreEqual(true, fieldInfo2.IsInitOnly);
        }
