...
namespace _01._Shortest_Path_in_Matrix
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var result = Dijkstra(matrix, 0, 0, rows - 1, columns - 1);
            Console.WriteLine("Length: " + result.Sum());
            Console.WriteLine("Path: " + string.Join(" ", result));
        }

        private static List<int> Dijkstra(int[,] graph, int sourceRow, int sourceCol, int destRow, int destCol)
        {
            ...
            var currentCell = new Tuple<int, int>(sourceRow, sourceCol);
            ...
            if (distance[destRow, destCol] == int.MaxValue)
            {
                return null;
            }

            var path = new List<int>();
            path.Add(graph[destRow, destCol]);
            var currentNode = previous[destRow, destCol];
            ...
            path.Add(graph[sourceRow, sourceCol]);
